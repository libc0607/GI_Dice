...
void on_bleuartServiceEvt(uint16_t connection_handle, ble_uart_evt_t *p_evt)
{
    switch(p_evt->type)
    {
        ...
        case BLE_UART_EVT_BLE_DATA_RECIEVED:
            ...
            app_drv_fifo_write(&app_uart_tx_fifo, (uint8_t *)p_evt->data.p_data, &(p_evt->data.length));
            break;
        default:
            break;
    }
...
static uint8 scanRspData[SCANRSP_DATA_LEN] = {
    // complete name
    9, // length of this data
    GAP_ADTYPE_LOCAL_NAME_COMPLETE,
    'D', 'i', 'c', 'e', '0', '0', '0', '0', // *** u should modify patch_mac_addr_to_ble_conf() ***
...
// about how to modify & update advertData, see https://www.cnblogs.com/debugdabiaoge/p/15909231.html
static uint8 advertData[ADVERT_DATA_LEN] = {
    ...
    BLE_UART_SERVICE_UUID_LO,
    BLE_UART_SERVICE_UUID_HI,

    // custom data: battery, orientation
    // *** u should also see dice_update_advert_data() since some dirty magic numbers ***
    5,
    GAP_ADTYPE_MANUFACTURER_SPECIFIC,
    'G', 'I',   // "Genius Invokation", custom header
    0x00,       // battery voltage x, uint8; Vbat(mV) = (4220-(4*x)), 3200mV~4220mV
    0x00       // dice orientation, lis_dice_8d_t
};

// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "GI Dice 0000"; // *** u should modify patch_mac_addr_to_ble_conf() ***
...
// a dirty patch to contain mac addr in ble name
void patch_mac_addr_to_ble_conf()
{
    uint8_t buf[6];

    if (0 != GetMACAddress(buf)) {
        PRINT("Error: can't get MAC address\n");
    } else {
        sprintf(scanRspData+6, "%X%X", buf[1], buf[0]);
        sprintf(attDeviceName+8, "%X%X", buf[1], buf[0]);
        //sprintf(advertData+9, "%X%X", buf[1], buf[0]);
    }
}

// *** dirty advertData & scanRspData updator ***
...
void dice_update_advert_data()
{
    ...
    bat_mv = (bat_mv > 4220)? 4220: bat_mv;
    bat_mv = (bat_mv < 3200)? 3200: bat_mv;
    bat_8b = (uint8_t)((bat_mv-3200)/4);

    advertData[ADVERT_BAT_OFFSET] = bat_8b;
    advertData[ADVERT_ORI_OFFSET] = ori;
    //scanRspData[SCANRSP_BAT_OFFSET] = bat_8b;
    //scanRspData[SCANRSP_ORI_OFFSET] = ori;
    GAP_UpdateAdvertisingData(0, TRUE, ADVERT_DATA_LEN, advertData);
    //GAP_UpdateAdvertisingData(0, FALSE, SCANRSP_DATA_LEN, scanRspData);
}
